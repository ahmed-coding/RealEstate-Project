    ...
    async def receive_json(self, content):
        ...
        try:
            if command == " ":
                ...
                if payload == None:
                    raise ClientError(
                        "UNKNOWN_ERROR", "Something went wrong. Try refreshing the browser.")
                else:
                    payload = json.loads(payload)
...
def accept_friend_request(user, notification_id):
    ...
    if user.is_authenticated:
        try:
            ...
            raise ClientError(
                "AUTH_ERROR", "An error occurred with that notification. Try refreshing the browser.")
    return None
...
def get_new_chat_notifications(user, newest_timestatmp):
    ...
    if user.is_authenticated:
        ...
        chatmessage_ct = ContentType.objects.get_for_model(
            UnreadChatRoomMessages)
        notifications = Notification.objects.filter(target=user, content_type__in=[
                                                    chatmessage_ct], timestamp__gt=timestamp).order_by('-timestamp')
        s = LazyNotificationEncoder()
        payload['notifications'] = s.serialize(notifications)
        return json.dumps(payload)
    else:
